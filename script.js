 ...
 * This is the fixed version to ensure robust storage key and price handling.
 */
function addToCart(itemId, qty, nameOverride = null, priceOverride = null) {
    ...
    // CRITICAL: Save the updated cart to storage
    saveCart(cart); 
    alert(`${qty} x ${itemDetails.name} added to cart!`);
    
    // Call the display update after the data is successfully saved
    if (document.getElementById('cart-items-list')) {
        renderOrderCartPreview();
    }
    // Added checkout update check for immediate feedback if user is still on checkout.html
...
function handleDelightsAddToCart(button) {
    ...
    // Selects input based on the .product-qty class added in the HTML fix
...
function renderCart(cartData) {
    ...
    if (cartData.length === 0) {
        ...
        cartData.forEach(item => {
            ...
            html += `
                <div class="item-row" style="display: grid; grid-template-columns: 2fr 0.5fr 1fr; gap: 10px; padding: 5px 0; border-top: 1px dashed #555;">
                    <span>${item.name}</span>
                    <span>${item.qty}</span>
...
// =================================================================
// 6. INITIALIZATION (Runs on both order.html and checkout.html)
// =================================================================
